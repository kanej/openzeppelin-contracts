...
describe('RSA', function () {
    ...
    for (const test of parse('./test/utils/cryptography/SigVer15_186-3.rsp')) {
      ...
      if (test.SHAAlg === 'SHA256' && length >= 0x100) {
        ...
        it(`signature length ${length} ${test.extra} ${result ? 'works' : 'fails'}`, async function () {
          ...
          const sig = '0x' + test.S;
          const exp = stripZerosLeft('0x' + test.e); // strip zeros to reduce gas cost of the precompile
          const mod = '0x' + test.n;

          expect(await this.mock.$pkcs1Sha256(Typed.bytes32(sha256(data)), sig, exp, mod)).to.equal(result);
  ...
  describe('others tests', function () {
    ...
    const sTooLarge = {
      ...
      sig: toBeHex(toBigInt(openssl.sig) + toBigInt(openssl.mod)),
      result: false,
    };
