...
describe('CrosschainRemoteController & CrosschainRemoteExecutor', function () {
  ...
  it('setup', async function () {
    ...
    await expect(this.executor.controller()).to.eventually.equal(this.helpers.chain.toErc7930(this.admin));
  });
  ...
  describe('crosschain operation', function () {
    it('support single mode', async function () {
      ...
      await expect(this.remoteExecute(this.admin, this.helpers.chain.toErc7930(this.executor), mode, data))
  ...
  describe('reconfigure', function () {
    ...
    it('through a crosschain call: success', async function () {
      ...
      const data = encodeSingle(
        ...
        this.executor.interface.encodeFunctionData('reconfigure', [
          ...
          this.helpers.chain.toErc7930(this.other),
        ]),
      );

      await expect(this.remoteExecute(this.admin, this.helpers.chain.toErc7930(this.executor), mode, data))
        .to.emit(this.executor, 'CrosschainControllerSet')
        .withArgs(this.newGateway, this.helpers.chain.toErc7930(this.other));

      await expect(this.executor.gateway()).to.eventually.equal(this.newGateway);
      await expect(this.executor.controller()).to.eventually.equal(this.helpers.chain.toErc7930(this.other));
    });

    it('through the internal setter: success', async function () {
      await expect(this.executor.$_setup(this.newGateway, this.helpers.chain.toErc7930(this.other)))
        .to.emit(this.executor, 'CrosschainControllerSet')
        .withArgs(this.newGateway, this.helpers.chain.toErc7930(this.other));
    ...
    it('with an invalid new gateway: revert', async function () {
      ...
      await expect(this.executor.$_setup(this.other, this.helpers.chain.toErc7930(this.other))).to.be.revert(ethers);
      ...
      await expect(
        this.remoteExecute(this.admin, this.helpers.chain.toErc7930(this.executor), mode, data),
      ).to.be.revertedWithCustomError(this.executor, 'FailedCall');
    });

    it('is access controlled', async function () {
      await expect(
        this.executor.reconfigure(this.newGateway, this.helpers.chain.toErc7930(this.other)),
