...
describe('GovernorCrosschain', function () {
      ...
      [
        {
          ...
          data: this.governor.interface.encodeFunctionData('relayCrosschain(address,bytes,bytes32,bytes)', [
            this.gateway.target,
            this.helpers.chain.toErc7930(this.executor),
  ...
  it('reconfigure executor', async function () {
    ...
    await expect(this.executor.controller()).to.eventually.equal(this.helpers.chain.toErc7930(this.governor));
          ...
          data: this.governor.interface.encodeFunctionData('relayCrosschain(address,bytes,bytes32,bytes)', [
            ...
            encodeSingle(
              ...
              this.executor.interface.encodeFunctionData('reconfigure', [
                this.gateway.target,
                this.helpers.chain.toErc7930(newGovernor),
              ]),
            ),
          ]),
        },
    ...
    await expect(this.helper.execute())
      ...
      .withArgs(this.gateway, this.helpers.chain.toErc7930(newGovernor));

    // After reconfiguration
    await expect(this.executor.gateway()).to.eventually.equal(this.gateway);
    await expect(this.executor.controller()).to.eventually.equal(this.helpers.chain.toErc7930(newGovernor));
  });
});
